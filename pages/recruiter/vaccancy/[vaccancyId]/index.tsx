        ...
        {data?.candidatesApplied.map((item: any) => (
          <div key={item.id} className={styles.applicant}>
            ...
            <div className={styles.footer}>
              <Button
                ...
                onClick={() =>
                  handleAccept(
                    ...
                    data?.recruiter?.rooms?.filter((room:any)=>room.candidateId ===item.id)[0].id,
                    data?.recruiter?.rooms?.filter((room:any)=>room.candidateId ===item.id)[0].name,
                  )
                }
                disabled={
              ...
              >
                {(loadingAccept && Number(item.id) === currentLoading)?(<CircularProgress size="20px" sx={{color:"white"}} />):item?.accepted?.jobsAccepted.filter(
                  (it: any) => Number(it.id) === Number(vaccancyId)
                ).length
                  ? "invited"
                  : "invite"}
              ...
              >
                ...
                {(loading && Number(item.id) === currentLoading)?(<CircularProgress size="20px" sx={{color:"white"}} />):item?.refused?.jobsRefused.filter(
                  (it: any) => Number(it.id) === Number(vaccancyId)
                ).length
                  ? "refused"
                  : " not ready"}
